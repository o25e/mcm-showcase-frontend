...
export default function ProductDetail() {
  ...
  return <div className="product-page" id="top">
    <Header />
    <main>
      <section className="product" aria-label="상품 상세">
        ...
        <div className="details">
          ...
          <button className="cart-button" type="button">쇼핑백에 담기</button>
          <button className={`wish-button ${isWished ? 'saved' : ''}`} type="button" onClick={() => setIsWished(!isWished)}><img src="/assets/icon-heart-small.svg" alt="" /><span>{isWished ? '찜 완료' : '찜하기'}</span></button>
        </div>
      </section>
      <section className="recommendations" aria-labelledby="recommend-title"><h2 id="recommend-title">함께 보면 좋은 상품</h2><div className="recommendation-grid">{recommendations.map((item) => <ProductCard key={item.name} {...item} />)}</div></section>
    </main>
  </div>;
}
