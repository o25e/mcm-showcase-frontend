...
export default function FittingPage({ onFinish, arSessionId, gender, language = 'ko' }) {
  ...
  return <section className={`fitting-page ${language === 'en' ? 'fitting-page--en' : ''}`} aria-labelledby="fitting-page-title">
    ...
    <div className="fitting-page__history" ref={historyRef} aria-label="피팅 히스토리">{visibleHistory.map((item, index) => <button className={`fitting-page__history-card ${item ? 'active' : ''}`} type="button" key={item ? item.id : `history-empty-${index}`} onClick={() => item && setAvatarImage(item.avatarImage)} disabled={!item}>{item && <img className="fitting-page__history-heart" src={item.wishlisted ? "/assets/product-detail-heart-click.png" : "/assets/product-detail-heart.png"} alt="" onClick={(event) => { event.stopPropagation(); toggleWishlist(item); }} />}{item && <img className="fitting-page__history-hanger" src={fittingProductIds.has(item.productId) ? "/assets/icon-cloth-click.png" : "/assets/product-detail-cloth.png"} alt="" onClick={(event) => { event.stopPropagation(); requestFitting(item); }} />}{item && <img className="fitting-page__history-product" src={item.imageUrl} alt={`${item.productName} 피팅 기록`} />}</button>)}</div>
