...
export default function AvatarCompletePage({
  ...
  */
  ...
  return (
    <main
      className={`avatar-complete-page ${language === 'en' ? 'avatar-complete-page--en' : ''}`}
      aria-labelledby="avatar-complete-title"
    >
      ...
      <nav className="avatar-complete-page__steps" aria-label="AR fitting progress">
        {steps.map((step) => (
          <span className="active" key={step}>{step}</span>
        ))}
      </nav>

      <div className="avatar-complete-page__track" aria-hidden="true">
        <span />
      </div>

      <section className="avatar-complete-page__content">
        <h1 id="avatar-complete-title">{avatarLook?.styleIdentityTitle || t.completeTitle}</h1>
        ...
        <img className="avatar-complete-page__avatar" src={avatarImage} alt="Today's avatar" />
      </section>

      <aside className="avatar-complete-page__preview" aria-label="Generated avatar preview">
        ...
        <button type="button" onClick={() => setIsFinishModalOpen(true)}>{t.finishAr}</button>
      </aside>
      ...
      <section className="avatar-complete-page__qr-card" aria-label="Fitting result information">
        ...
        {language === 'en' ? (
          <p>
            <span className="qr-line">Scan the QR code to revisit today's style online</span>
            <span className="qr-line">or on your next visit to MCM.</span>
          </p>
        ) : (
          <p>
      ...
      {isFinishModalOpen && (
        <section
          className="avatar-complete-finish-modal"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="avatar-complete-finish-modal__card">
            ...
            <p id="finish-modal-title">{t.tryout}</p>
            ...
            <button
              className="avatar-complete-finish-modal__confirm"
              type="button"
              onClick={onFinish}
            >
              {t.confirmFinish}
