...
export default function AvatarCompletePage({ avatarImage = '/assets/avatar-complete/avatar.png', onFinish, language = 'ko' }) {
  ...
  return <main className={`avatar-complete-page ${language === 'en' ? 'avatar-complete-page--en' : ''}`} aria-labelledby="avatar-complete-title">
    ...
    <section className="avatar-complete-page__content"><h1 id="avatar-complete-title">{t.completeTitle}</h1><img className="avatar-complete-page__avatar-background" src="/assets/avatar-complete/avatar-background.svg" alt="" aria-hidden="true" /><img className="avatar-complete-page__avatar" src={avatarImage} alt="Today's avatar" /></section>
    ...
    {isFinishModalOpen && <section className="avatar-complete-finish-modal" role="dialog" aria-modal="true" aria-labelledby="finish-modal-title"><button className="avatar-complete-finish-modal__backdrop" type="button" aria-label="Close finish information" onClick={() => setIsFinishModalOpen(false)} /><div className="avatar-complete-finish-modal__card"><img className="avatar-complete-finish-modal__logo" src="/assets/MCM-logo.png" alt="MCM" /><span className="avatar-complete-finish-modal__rule" aria-hidden="true" /><p id="finish-modal-title">{t.tryout}</p><div className="avatar-complete-finish-modal__message">{language === 'en' ? <span>{t.showcase}</span> : <><span className="finish-line">기다리는 동안 3F 전광판에서</span><strong className="finish-line finish-line--bold">MCM의 새로운 쇼케이스를 만나보세요.</strong></>}</div><button className="avatar-complete-finish-modal__confirm" type="button" onClick={onFinish}>{t.confirmFinish}</button></div></section>}
  </main>;
}
