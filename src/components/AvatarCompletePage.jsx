...
export default function AvatarCompletePage({ 
  ...
  useEffect(() => { 
    console.log('AvatarCompletePage arSessionId:', arSessionId);

    if (arSessionId == null) return; 
    if (hasCreatedAvatarLook.current) return;
    ...
    const createAvatarLook = async () => { 
      try { 
        const response = await fetch( 
          `https://api.mcm-showcase.com/api/recommendations/avatar-look/${arSessionId}`, 
          { 
            method: 'POST',
            headers: {
              Accept: '*/*',
        ...
        console.log('아바타 룩 API status:', response.status);

        if (!response.ok) { 
          const errorText = await response.text();
          console.error('아바타 룩 서버 응답:', errorText);
          throw new Error(`아바타 룩 생성 실패: ${response.status}`); 
        } 
 
        const data = await response.json(); 
 
        console.log('아바타 룩 생성 성공:', data); 
        console.log('생성된 styleProfileId:', data.styleProfileId); 
        console.log('styleIdentityTitle:', data.styleIdentityTitle); 
        console.log('선택된 products:', data.products); 
 
        setAvatarLook(data); 
      } catch (error) { 
        console.error('아바타 룩 생성 error:', error); 
      } 
    }; 
  ...
  return ( 
    <main className="avatar-complete-page" aria-labelledby="avatar-complete-title"> 
      ...
      <section className="avatar-complete-page__content"> 
        <h1 id="avatar-complete-title">{avatarLook?.styleIdentityTitle || '오늘의 스타일이 완성됐어요.'}</h1> 
        <img className="avatar-complete-page__avatar-background" src="/assets/avatar-complete/avatar-background.svg" alt="" aria-hidden="true" /> 
      ...
      {isFinishModalOpen && ( 
        <section className="avatar-complete-finish-modal" role="dialog" aria-modal="true" aria-labelledby="finish-modal-title"> 
          <button className="avatar-complete-finish-modal__backdrop" type="button" aria-label="종료 안내 닫기" onClick={() => setIsFinishModalOpen(false)} /> 
          ...
          <div className="avatar-complete-finish-modal__card"> 
            ...
            <button className="avatar-complete-finish-modal__confirm" type="button" onClick={onFinish}>확인 및 종료하기</button> 
          </div> 
        </section> 
      )} 
    </main> 
