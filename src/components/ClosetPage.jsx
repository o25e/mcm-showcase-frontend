...
export default function ClosetPage() {
  ...
  const startHistoryDrag = (event) => {
    ...
    const endHistoryDrag = () => {
      ...
      window.removeEventListener('mousemove', dragHistory, true);
      window.removeEventListener('mouseup', endHistoryDrag, true);
    };
    window.addEventListener('mousemove', dragHistory, true);
    window.addEventListener('mouseup', endHistoryDrag, true);
  };
      ...
      <aside className="closet-detail-sheet" role="dialog" aria-modal="true" aria-labelledby="closet-detail-title">
        ...
        <div className="closet-detail-content">
          ...
          <div className="closet-detail-main">
            <header className="closet-detail-header"><p>{selectedRecord.date}</p><h2 id="closet-detail-title">{selectedRecord.title}</h2><span><img src="/assets/icon-place.svg" alt="" /> 이태원 플래그십</span><div className="closet-detail-stats"><span>♧ 2</span><span>♡ 3</span></div></header>
            <section className="closet-outfit"><h3>오늘의 룩</h3><div className="closet-product-today"><img src={closetProducts[0].image} alt={closetProducts[0].name} /><button type="button" aria-label="상품 찜하기">♡</button><p>{closetProducts[0].name}</p><small>{closetProducts[0].price}</small></div></section>
            <section className="closet-history"><h3>HISTORY</h3><div className="closet-history-carousel"><div className="closet-history-list" ref={historyRef} onMouseDown={startHistoryDrag}>{closetProducts.map((product, index) => <article className="closet-history-card" key={index}><div><img src={product.image} alt={product.name} draggable="false" /><button type="button" aria-label="상품 찜하기">♡</button></div><p>{product.name}</p><small>{product.price}</small></article>)}</div><span className="closet-history-next" aria-hidden="true"><img src="/assets/icon-next.png" alt="" /></span></div></section>
          </div>
        </div>
      </aside>
    </div>}
