...
export default function FittingHelpOverlay({ onClose }) {
  return <section className="fitting-help" aria-labelledby="fitting-help-title">
    ...
    <section className="fitting-help__catalog" aria-label="추천 상품"><div className="fitting-help__tabs">{categories.map((category, index) => <span className={index === 0 ? 'active' : ''} key={category}>{category}</span>)}</div><div className="fitting-help__catalog-rule" aria-hidden="true" /><div className="fitting-help__product-grid">{products.map((product, index) => <article className="fitting-help__product" key={product}><img src={product} alt={`추천 상품 ${index + 1}`} /></article>)}</div><button className="fitting-help__refresh" type="button" aria-label="새로고침"><img src="/assets/figma-fitting-refresh.svg" alt="" /></button></section>
